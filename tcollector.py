...
class Collector(object):
	...
	def read(self):
		...
		try:
			...
			print ('***  IOError:', xxx_todo_changeme1) #!!!
...
def main(argv):
	...
	tagstr = ''
	if tags:
		tagstr = ' '.join('%s=%s' % (k, v) for k, v in tags.items())
		tagstr = ' ' + tagstr.strip()
...
def reload_changed_config_modules(modules, options, sender, tags):
	...
	if changed:
		sender.tagstr = ' '.join('%s=%s' % (k, v) for k, v in tags.items())
...
def spawn_collector(col):
	...
	try:
		col.proc = subprocess.Popen(col.filename, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
	except OSError as e:
